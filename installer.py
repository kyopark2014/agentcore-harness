...
CloudFront, IAM roles, S3 Vectors Knowledge Base, KB + artifact-share
MCP Runtimes behind a shared AgentCore Gateway, CreateHarness) for local
development.
"""
...
VPC_CIDR = "10.52.0.0/16"  # used by legacy inline VPC helpers if kept
...
KB_MCP_DIR = os.path.join(WORKING_DIR, "MCP", "knowledge-base")
ARTIFACT_SHARE_MCP_DIR = os.path.join(WORKING_DIR, "MCP", "artifact-share")
...
def load_config(config_path: str) -> Dict:
    ...
    global s3files_client, s3vectors_client
...
def knowledge_base_mcp_url(agent_runtime_arn: str, runtime_region: Optional[str] = None) -> str:
...
def ensure_harness_sharing_env(
    ...
    """Inject S3_BUCKET / SHARING_URL / SESSION_STORAGE_DIR / KB ids for runtime."""
...
def build_config_from_deployment_state(
    ...
    # Remove legacy Memory / old s3-sharing MCP keys on write (Memory not used)
    for legacy in (
...
def main():
    ...
    try:
        ...
        logger.info("[4/6] Ensuring VPC for Harness (S3 Files requires VPC mode)")
        vpc_info = provisioner.ensure_vpc()
        logger.info("[5/6] Creating S3 Files session storage")
        ...
        logger.info(
            f"  Knowledge Base MCP URL: "
            f"{(knowledge_base_mcp_info or {}).get('knowledge_base_mcp_url')}"
        )
        logger.info(
            f"  Artifact Share MCP URL: "
            f"{(artifact_share_mcp_info or {}).get('artifact_share_mcp_url')}"
        )
